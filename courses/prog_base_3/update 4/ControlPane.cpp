...
void Pane::addDoubleVar(const char * varName, double * var, bool isNonnegat, bool ableToSetDefault, bool readOnly)
{
	char strBuff[256], strBuff1[256];

	sprintf(strBuff, "%s_mantissa", varName);
	sprintf(strBuff1, "label='mantissa' group='%s' min=%s max=10.0 step=0.02 readonly='%s'", varName, isNonnegat ? "0.0" : "-10.0", readOnly ? "true" : "false");
	TwAddVarCB(twBar, strBuff, TW_TYPE_DOUBLE, TwCB::setMantissa, TwCB::getMantissa, var, strBuff1);

	sprintf(strBuff, "%s_exponent", varName);
	sprintf(strBuff1, "label='exponent' group='%s' readonly='%s'", varName, readOnly ? "true" : "false");
...
void Pane::addVector3dVar(const char * varName, Vector3d * var, bool isNonnegat, bool ableToSetDefault, bool readOnly) {
	char strBuff[256], strBuff1[256];

	addDoubleVarToGroup(varName, "x", &var->x, isNonnegat, ableToSetDefault, readOnly);
	addDoubleVarToGroup(varName, "y", &var->y, isNonnegat, ableToSetDefault, readOnly);
	addDoubleVarToGroup(varName, "z", &var->z, isNonnegat, ableToSetDefault, readOnly);
...
void Pane::addDoubleVarToGroup(const char * groupName, const char * varName, double * var, bool isNonnegat, bool ableToSetDefault, bool readOnly)
{
	char strBuff[256], strBuff1[256];

	sprintf(strBuff, "%s_%s", groupName, varName);
	addDoubleVar(strBuff, var, isNonnegat, ableToSetDefault, readOnly);
...
void Pane::addVector3dVarToGroup(const char * groupName, const char * varName, Vector3d * var, bool isNonnegat, bool ableToSetDefault, bool readOnly)
{
	char strBuff[256], strBuff1[256];

	sprintf(strBuff, "%s_%s", groupName, varName);
	addVector3dVar(strBuff, var, isNonnegat, ableToSetDefault, readOnly);
...
void Pane::addSpaceObject(SpaceObject * spObj, bool ableToSetDefault, bool ableToDelete, bool closed)
{
	...
	addVector3dVarToGroup(strBuff, "gravityAcceleration", &spObj->gravityAccel, false, false, true);
	addVector3dVarToGroup(strBuff, "additionalAcceleration", &spObj->additionalAccel, false);
	addVector3dVarToGroup(strBuff, "resultAcceleration", &spObj->resultAccel, false, false, true);
...
ControlPane::ControlPane(Camera * camera, RenderManager * renderManager) {
	...
	addDoubleVarToGroup("factors", "teapotsDiameterDisplayFactor", &Teapot::diamDisplayFactor, true);
