...
namespace TwCB {
	...
	void TW_CALL setDefaultTeapot_onButtonClick(void * clientData) {
		VarWithDefault<Teapot> * v = static_cast<VarWithDefault<Teapot> *>(clientData);
		*v->var = v->defaultVal;
	}
		...
		for (void * spObjWithDefault : *v) {
			...
			if (spObj->getType() == STAR) {
				...
				*sputnikWithDefault->var = sputnikWithDefault->defaultVal;
				...
				VarWithDefault<Teapot> * teapotWithDefault = (VarWithDefault<Teapot> *)spObjWithDefault;
				*teapotWithDefault->var = teapotWithDefault->defaultVal;
			}
		}
	}
	...
	void TW_CALL showNewSpaceObjectPane_sputnik_onButtonClick(void * clientData) {
		SpaceObjectAndPane * spObjAndPane = static_cast<SpaceObjectAndPane *>(clientData);
		NewSpaceObjectPane * pane = dynamic_cast<NewSpaceObjectPane *>(spObjAndPane->pane);
		SpaceObject * parent = spObjAndPane->spObj;

		if (pane->isVisible()) return;

		pane->addNewSpaceObject(SPUTNIK, parent);
		pane->show();
	}
	...
	void TW_CALL changePlanetTexture_onButtonClick(void * clientData) {
		Planet * planet = static_cast<Planet *>(clientData);
		planet->setNextTexture();
	}
}