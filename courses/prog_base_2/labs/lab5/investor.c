...
void Investor_toXmlNode(Investor_t self, xmlDocPtr doc, xmlNodePtr parentNode){
    ...
    xmlNodePtr contactsNode = xmlNewChild(investorNode, NULL, "contacts", NULL);

    xmlNodePtr n = xmlNewChild(contactsNode, NULL, "telephoneNumber", self->contacts.telephoneNumber);
    xmlNewChild(contactsNode, NULL, "address", self->contacts.address);
    xmlNewChild(contactsNode, NULL, "email", self->contacts.email);
}
...
void Investor_update(Investor_t self, const char * key, const char * value){
    if(!strcmp(key, "name")){
        ...
        strcpy(self->contacts.telephoneNumber, value);
    }else if(!strcmp(key, "address")){
        strcpy(self->contacts.address, value);
    }else if(!strcmp(key, "email")){
        strcpy(self->contacts.email, value);
    }
}

char * Investor_toString(Investor_t self, char * mem_p){
    sprintf(mem_p, "Investor %s %s:\nbirthdate is %s;\ninvested %f grivnas in %d projects. \n"
            "Contacts: telephone number: %s; address: %s, email: %s", self->name,
           self->surname, self->birthdate, self->investment, self->projectsNumber,
           self->contacts.telephoneNumber, self->contacts.address, self->contacts.email);
    return mem_p;
}
