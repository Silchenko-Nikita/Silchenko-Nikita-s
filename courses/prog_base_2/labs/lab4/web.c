...
Web_t Web_new(int port){
    ...
    DataHandler_deserializeInvestors(self->dataHandler);
        ...
        if(!strcmp(child2, "")){
            ...
            if(_checkIndex(self, index) == SERVER_INDEX_ERROR){
                ...
                if (method == HTTP_GET){
                    ...
                    if(_checkArgs(httpRequest, clientSock) == SERVER_OK){
                        for(int i = 0; i < argsNum; i++){
                            const char * key = HttpRequest_getKey(httpRequest, i);
                            const char * val = HttpRequest_getArg(httpRequest, key);

                            DataHandler_updateInvestor(self->dataHandler, index, key, val);
                        }
                        DataHandler_serializeInvestors(self->dataHandler);
...
static void _writeInvestors_HTML(Web_t self, socket_t * clientSock){
    ...
    for(int i = 0; i < invsNum; i++){
        char buff2[256];
        Investor_t inv = DataHandler_getInvestor(self->dataHandler, i);
        sprintf(buff2, "<li><a href=\"/investors/%d\">%s %s<a></li>", i, inv->name, inv->surname);
        strcat(buff, buff2);
    }
...
static SERVER_ERROR _checkIndex(Web_t self, int index){
    if(index < 0 || index >= DataHandler_getInvestorsNum(self->dataHandler)) return SERVER_INDEX_ERROR;
